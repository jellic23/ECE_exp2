        ...
        if(6'b001000<=ticking[17:12] && ticking[17:12] <= 6'b010110)  begin                                                             //주간
           case(light_time_state)
               0 : begin                                                            //state A* 보행점멸 구현 완료..
                   if((TIME_SCALE*5)/2 <= flickering) begin                                                       
                       if((TIME_SCALE*5)/2 <= flickering && flickering < (TIME_SCALE*6)/2) begin
                           west_p = 2'b00; east_p = 2'b00;
                       end
                       else if((TIME_SCALE*6)/2 <= flickering && flickering < (TIME_SCALE*7)/2) begin
                           west_p = 2'b10; east_p = 2'b10;
                       end
                       else if((TIME_SCALE*7)/2 <= flickering && flickering < (TIME_SCALE*8)/2) begin
                           west_p = 2'b00; east_p = 2'b00;
                       end
                       else if((TIME_SCALE*8)/2 <= flickering && flickering < (TIME_SCALE*9)/2) begin
                           west_p = 2'b10; east_p = 2'b10;
                           south = 4'b0100; north = 4'b0100;
                       end
                       else if((TIME_SCALE*9)/2 <= flickering && flickering < (TIME_SCALE*10)/2) begin
                           west_p = 2'b00; east_p = 2'b00;
                           south = 4'b0100; north = 4'b0100;
                       end 
                   end
               ...
               1 : begin                                                            //state D* 보행점멸 없음.
                   if(TIME_SCALE*4 <= flickering) begin
                       if(TIME_SCALE*4 <= flickering && flickering < TIME_SCALE*4 + TIME_SCALE/4) begin
                           south = 4'b0011; north = 4'b0011;
                       end
                       else if(TIME_SCALE*4 + TIME_SCALE/4 <= flickering && flickering < TIME_SCALE*4 + (TIME_SCALE*2)/4) begin
                           south = 4'b0110; north = 4'b0110;
                       end
                       else if(TIME_SCALE*4 + (TIME_SCALE*2)/4 <= flickering && flickering < TIME_SCALE*4 + (TIME_SCALE*3)/4) begin
                           south = 4'b0110; north = 4'b0110;
                       end
                       else if(TIME_SCALE*4 + (TIME_SCALE*3)/4 <= flickering && flickering < TIME_SCALE*4 + (TIME_SCALE*4)/4) begin
           ...
           endcase
               ...
               case(light_time_state)
                   0 : begin                                                            //state B* 보행점멸 구현 완료 
                       if((TIME_SCALE*10)/2 <= flickering) begin                                                       
                           ...
                           else if((TIME_SCALE*18)/2 <= flickering && flickering < (TIME_SCALE*19)/2) begin
                               east_p = 2'b00;
                               north = 4'b1100;
                           end
                           else if((TIME_SCALE*19)/2 <= flickering && flickering < (TIME_SCALE*20)/2) begin
                               east_p = 2'b10;
                               north = 4'b1100;
                           end
                       end
...
/*                           if((TIME_SCALE*10)/2 <= flickering && flickering < (TIME_SCALE*11)/2) begin
                           ...
                           if((TIME_SCALE*18)/2 <= flickering && flickering < (TIME_SCALE*19)/2) begin
    ...
    if(!rst) begin
        ...
        if(emergency_indicate_int_1==1) begin
            if(btn_t == 4'b1000 || btn_t == 4'b0100 || btn_t == 4'b0010 || btn_t == 4'b0001) begin
                ...
                if(emergency_clkby==TIME_SCALE) emergency_clkby<=1;
    ...
    if(!rst) begin
        ...
        if(1 <= emergency_buffering && emergency_buffering <= 15) begin
            ...
            if(6'b001000<=ticking[17:12] && ticking[17:12] <= 6'b010110)  begin  //주간
                ...
                if(ta==TIME_SCALE) light_time = light_time + 1;
            
                if(ticking == 18'b001000_000000_000000) begin
                    light_time = 0;
                    light_time_state = 0;
                    flickering=98;
                end
                ...
                if(addhour_btn_t==1 && ticking[17:12] == 6'b000111) light_time = light_time - 5;
                if(ticking == 18'b010111_000000_000000) begin
                    light_time = 0;
                    light_time_state = 0;
                    flickering=98;
                end
